...
public class KhachHangDAO implements IDAO<KhachHang> {
	...
	// -----------------------
	public KhachHang selectByUseAndPassWord(KhachHang t) {
		KhachHang ketQua = null;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
	...
	@Override
	public int daleteAll(ArrayList<KhachHang> arr) {
		int dem = 0;
		for (KhachHang KhachHang : arr) {
	...
	//-------------------------------------
	public boolean capNhatMatKhau(KhachHang t) {
		int ketQua = 0;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
	...
	//---------------------------------------------
	public int updateThongTin(KhachHang t) {
		int ketQua = 0;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
